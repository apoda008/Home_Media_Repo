...
int main() {
    ...
    if (file_check != NULL) {
        ...
        if (database == NULL) {
            printf("Database failed to load\n");
        }
        fclose(file_check);
        ...
        database = Construct_Database_Structure(master_pathing.num_of_files, global_ptr->num_of_files); 
    ...
    printf("Size of the database = %d\n", database->movie_set_size);
    printf("\n");
